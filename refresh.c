...
void randomise_one_bar(char** tab, int x){
	array[x].s1 = rand() % 25;
	array[x].s2 = rand() % 24;
	array[x].e1 = rand() % (25 - array[x].s1) + array[x].s1;
	array[x].e2 = rand() % (24 - array[x].s2) + array[x].s2;
...
void refresh_tab(char** tab, int n, int m){
	...
	for(i = 0; i < n; i++){
		...
		if(array[i].t1 < array[i].reset1)
			array[i].t1++;
		// if(array[i].t1 == array[i].reset1 || array[i].deleted1 == 1)
...
void randomise(int n){
	...
	for(i = 0; i < n; i++){
		array[i].s1 = rand() % 25;
		array[i].s2 = rand() % 24;
		array[i].e1 = rand() % (25 - array[i].s1) + array[i].s1;
